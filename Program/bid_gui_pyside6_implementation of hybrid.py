...
SORT_MODE_DESCRIPTIONS = {
    "weighted": "Auto-weight this row based on its position in the selected priority list.",
    "equal": "Give this row the same weight as the previous weighted/equal row.",
    "strict": "Make this row a hard priority break; following rows are used as tie-breakers after it.",
}
...
class BidGUI(QMainWindow):
    ...
    def _build_sorting_section(self, container: QVBoxLayout) -> None:
        ...
        selected_label = QLabel("Selected sorting priority")
        selected_label.setToolTip(
            "Each selected rule becomes: (column, direction, mode).\n\n"
            "weighted = auto-weight by position\n"
            "equal = same weight as the previous rule\n"
            "strict = hard priority/tie-breaker break"
        )
        grid.addWidget(available_label, 0, 0)
        grid.addWidget(selected_label, 0, 2)

        self.available_columns_list = QListWidget()
        self.available_columns_list.setMinimumHeight(210)

        self.selected_sort_list = QTableWidget(0, 3)
        self.selected_sort_list.setHorizontalHeaderLabels(["Column", "Direction", "Mode"])
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        ...
        self.selected_sort_list.setMinimumHeight(210)
        self.selected_sort_list.setToolTip(
            "The table is passed to sort_dataframe_by_conditions as 3-part rules, for example:\n"
        ...
        add_high = QPushButton("Add high weighted →")
        add_low = QPushButton("Add low weighted →")
        move_up = QPushButton("Move up")
        move_down = QPushButton("Move down")
        flip_direction = QPushButton("Flip high/low")
        remove = QPushButton("Remove")
        clear = QPushButton("Clear")
        set_weighted = QPushButton("Set weighted")
        set_equal = QPushButton("Set equal")
        set_strict = QPushButton("Set strict")
        advanced = QPushButton("Advanced settings...")

        add_high.setToolTip("Add the selected column as high-to-low using weighted mode.")
        add_low.setToolTip("Add the selected column as low-to-high using weighted mode.")
        set_weighted.setToolTip(SORT_MODE_DESCRIPTIONS["weighted"])
        set_equal.setToolTip(SORT_MODE_DESCRIPTIONS["equal"] + " Equal mode should not be the first rule.")
        set_strict.setToolTip(SORT_MODE_DESCRIPTIONS["strict"])
        advanced.setToolTip("Global settings for the auto-weight curve and whether helper score columns remain in Excel.")

        add_high.clicked.connect(lambda: self._add_sort_column("desc", "weighted"))
        add_low.clicked.connect(lambda: self._add_sort_column("asc", "weighted"))
        ...
        set_weighted.clicked.connect(lambda: self._set_selected_sort_mode("weighted"))
        set_equal.clicked.connect(lambda: self._set_selected_sort_mode("equal"))
        set_strict.clicked.connect(lambda: self._set_selected_sort_mode("strict"))
        ...
        for button in (move_up, move_down, flip_direction, remove, clear):
            sort_buttons.addWidget(button)
        line2 = QFrame()
        line2.setFrameShape(QFrame.HLine)
        sort_buttons.addWidget(line2)
        mode_label = QLabel("Selected row mode")
        mode_label.setStyleSheet(f"color: {UPS_GOLD}; font-weight: bold;")
        sort_buttons.addWidget(mode_label)
        for button in (set_weighted, set_equal, set_strict):
            sort_buttons.addWidget(button)
        line3 = QFrame()
        line3.setFrameShape(QFrame.HLine)
        sort_buttons.addWidget(line3)
        ...
        help_text = QLabel(
            "Hybrid sort example: Extra Vacation Days weighted → Training weighted → "
            "Blockiness equal → Premium strict."
        )
    ...
    def _set_selected_sort_mode(self, mode: str) -> None:
        ...
        if mode == "equal" and row == 0:
            QMessageBox.warning(
                self,
                "Equal mode needs a previous rule",
                "Equal mode means this row gets the same weight as the previous row, so it cannot be the first sorting rule.",
            )
            return
        ...
        self._refresh_selected_sort_list(select_index=row)
        self._clear_bid_string("Sorting mode changed. Generate the bid string again.")
    ...
    def _refresh_selected_sort_list(self, select_index: int | None = None) -> None:
        ...
        for row, (col, direction, mode) in enumerate(self.sort_order):
            ...
            direction_item = QTableWidgetItem(SORT_DIRECTIONS.get(direction, direction))
            mode_item = QTableWidgetItem(mode)

            mode_item.setToolTip(SORT_MODE_DESCRIPTIONS.get(mode, ""))

            self.selected_sort_list.setItem(row, 0, col_item)
            self.selected_sort_list.setItem(row, 1, direction_item)
            self.selected_sort_list.setItem(row, 2, mode_item)

        if select_index is not None and 0 <= select_index < len(self.sort_order):
            self.selected_sort_list.selectRow(select_index)
            self.selected_sort_list.setCurrentCell(select_index, 0)
    ...
    def _validated_sort_order_for_processing(self) -> list[tuple[str, str, str]]:
        ...
        for index, rule in enumerate(sort_order):
            ...
            if mode == "equal" and index == 0:
                raise ValueError(
                    f"{col} is the first sorting rule and cannot use equal mode. "
                    "Use weighted or strict for the first rule."
                )
