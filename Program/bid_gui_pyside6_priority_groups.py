...
WEIGHTING_STYLE_DESCRIPTIONS = {
    "soft": "Softer position-based auto-weights. With defaults, four weighted items become about 3.0, 2.33, 1.67, 1.0.",
    "hard": "Position-based auto-weights. Earlier weighted rows matter much more, for example 4, 3, 2, 1.",
}

SORT_MODE_DESCRIPTIONS = {
    "weighted": "Starts a new priority line. The program auto-weights it based on where it appears.",
    "equal": "Keeps this column on the same priority line as the one above it.",
    "strict": "Starts the tie-breaker section. Items here are used only after the main blended priorities are sorted.",
}

SORT_MODE_LABELS = {
    "weighted": "New priority",
    "equal": "Same priority",
    "strict": "Tie-breaker",
}
...
SORT_DIRECTION_FROM_LABEL = {label: value for value, label in SORT_DIRECTIONS.items()}

COMMON_SORT_TEMPLATE = [
    ("Extra Vacation Days", "desc", "weighted"),
    ("Training", "desc", "weighted"),
    ("Blockiness", "desc", "equal"),
    ("% tickets paid", "desc", "weighted"),
    ("Total DO", "desc", "weighted"),
    ("Avg # of legs", "asc", "equal"),
    ("Premium", "desc", "strict"),
]
...
class BidGUI(QMainWindow):
    ...
    def _build_sorting_section(self, container: QVBoxLayout) -> None:
        ...
        selected_label = QLabel("Your sorting recipe")
        selected_label.setToolTip(
            "Think of this as a recipe, not code:\n\n"
            "New priority = starts a new line in the main blended score.\n"
            "Same priority = stays on the same visual line as the item above.\n"
            "Tie-breaker = only used after the main priorities are sorted."
        )
        grid.addWidget(available_label, 0, 0)
        grid.addWidget(selected_label, 0, 2)

        self.available_columns_list = QListWidget()
        self.available_columns_list.setMinimumHeight(260)
        self.available_columns_list.setToolTip("Columns available for sorting after the PDFs are loaded.")

        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        right_layout.setContentsMargins(0, 0, 0, 0)

        visual_label = QLabel("Visual order")
        visual_label.setStyleSheet(f"color: {UPS_GOLD}; font-weight: bold;")
        right_layout.addWidget(visual_label)

        self.sort_visual_text = QTextEdit()
        self.sort_visual_text.setReadOnly(True)
        self.sort_visual_text.setMinimumHeight(135)
        self.sort_visual_text.setPlaceholderText(
            "Add sorting columns to build a visual recipe.\n"
            "Example: Training + Blockiness on one line means they share the same priority."
        )
        right_layout.addWidget(self.sort_visual_text)

        detail_label = QLabel("Detailed rules")
        detail_label.setStyleSheet(f"color: {UPS_GOLD}; font-weight: bold;")
        right_layout.addWidget(detail_label)

        self.selected_sort_list = QTableWidget(0, 4)
        self.selected_sort_list.setHorizontalHeaderLabels(["Line", "Column", "Direction", "Meaning"])
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.selected_sort_list.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeToContents)
        self.selected_sort_list.verticalHeader().setVisible(False)
        self.selected_sort_list.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.selected_sort_list.setSelectionMode(QAbstractItemView.SingleSelection)
        self.selected_sort_list.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.selected_sort_list.setMinimumHeight(190)
        self.selected_sort_list.setToolTip(
            "This still gets passed to the backend as weighted/equal/strict, "
            "but the GUI shows it in plain language."
        )
        right_layout.addWidget(self.selected_sort_list)

        grid.addWidget(self.available_columns_list, 1, 0)
        grid.addWidget(right_widget, 1, 2)

        sort_buttons_widget = QWidget()
        sort_buttons = QVBoxLayout(sort_buttons_widget)
        sort_buttons.setContentsMargins(0, 0, 0, 0)

        add_new_high = QPushButton("New priority: prefer high →")
        add_new_low = QPushButton("New priority: prefer low →")
        add_same_high = QPushButton("Same line: prefer high →")
        add_same_low = QPushButton("Same line: prefer low →")
        add_tie_high = QPushButton("Tie-breaker: prefer high →")
        add_tie_low = QPushButton("Tie-breaker: prefer low →")
        move_up = QPushButton("Move up")
        move_down = QPushButton("Move down")
        flip_direction = QPushButton("Flip high/low")
        remove = QPushButton("Remove")
        clear = QPushButton("Clear")
        set_new = QPushButton("Change to new priority")
        set_same = QPushButton("Change to same line")
        set_tie = QPushButton("Change to tie-breaker")
        common_setup = QPushButton("Use common bid setup")
        advanced = QPushButton("Advanced settings...")

        add_new_high.setToolTip("Add the selected column as its own priority line. Higher values are better.")
        add_new_low.setToolTip("Add the selected column as its own priority line. Lower values are better.")
        add_same_high.setToolTip("Add the selected column on the same visual line as the previous priority. Higher values are better.")
        add_same_low.setToolTip("Add the selected column on the same visual line as the previous priority. Lower values are better.")
        add_tie_high.setToolTip("Add the selected column to the tie-breaker section. Higher values are better.")
        add_tie_low.setToolTip("Add the selected column to the tie-breaker section. Lower values are better.")
        common_setup.setToolTip("Build a starter recipe using your common columns, when they exist in the loaded DataFrame.")
        advanced.setToolTip("Global settings for the auto-weight curve and whether helper score columns remain in Excel.")

        add_new_high.clicked.connect(lambda: self._add_sort_column("desc", "weighted"))
        add_new_low.clicked.connect(lambda: self._add_sort_column("asc", "weighted"))
        add_same_high.clicked.connect(lambda: self._add_sort_column("desc", "equal"))
        add_same_low.clicked.connect(lambda: self._add_sort_column("asc", "equal"))
        add_tie_high.clicked.connect(lambda: self._add_sort_column("desc", "strict"))
        add_tie_low.clicked.connect(lambda: self._add_sort_column("asc", "strict"))
        move_up.clicked.connect(self._move_sort_up)
        move_down.clicked.connect(self._move_sort_down)
        flip_direction.clicked.connect(self._flip_selected_sort_direction)
        remove.clicked.connect(self._remove_sort_column)
        clear.clicked.connect(self._clear_sort_order)
        set_new.clicked.connect(lambda: self._set_selected_sort_mode("weighted"))
        set_same.clicked.connect(lambda: self._set_selected_sort_mode("equal"))
        set_tie.clicked.connect(lambda: self._set_selected_sort_mode("strict"))
        common_setup.clicked.connect(self._use_common_sort_setup)
        advanced.clicked.connect(self._open_sorting_settings_dialog)

        section_label = QLabel("Add selected column")
        section_label.setStyleSheet(f"color: {UPS_GOLD}; font-weight: bold;")
        sort_buttons.addWidget(section_label)
        for button in (add_new_high, add_new_low, add_same_high, add_same_low, add_tie_high, add_tie_low):
        ...
        edit_label = QLabel("Edit selected rule")
        edit_label.setStyleSheet(f"color: {UPS_GOLD}; font-weight: bold;")
        sort_buttons.addWidget(edit_label)
        for button in (move_up, move_down, flip_direction, set_new, set_same, set_tie, remove, clear):
            sort_buttons.addWidget(button)

        line2 = QFrame()
        line2.setFrameShape(QFrame.HLine)
        sort_buttons.addWidget(line2)
        sort_buttons.addWidget(common_setup)
        ...
        help_text = QLabel(
            "Plain-English mapping: New priority = weighted, Same line = equal, Tie-breaker = strict. "
            "For example, putting Training and Blockiness on the same line makes them share the same priority."
        )
        help_text.setWordWrap(True)
        help_text.setStyleSheet(f"color: {UPS_TEXT}; font-style: italic;")
        grid.addWidget(help_text, 2, 0, 1, 3)

        self.default_mode = str(DEFAULT_SORTING_SETTINGS.get("default_mode", "weighted"))
    ...
    def _load_saved_values(self) -> None:
        ...
        if isinstance(saved_sort_order, list):
            normalized_saved_sort_order: list[list[str]] = []
            for item in saved_sort_order:
                normalized_rule = self._normalize_sort_rule(item)
                if normalized_rule is not None:
                    normalized_saved_sort_order.append(normalized_rule)
            self.sort_order = normalized_saved_sort_order
    ...
    def _normalize_sort_rule(self, rule: Any) -> list[str] | None:
        if not isinstance(rule, (list, tuple)) or len(rule) < 2:
            return None

        col = str(rule[0])
        direction = str(rule[1]).lower()
        mode = str(rule[2]).lower() if len(rule) >= 3 else "weighted"

        if direction not in SORT_DIRECTIONS:
            direction = "desc"
        if mode not in SORT_MODE_DESCRIPTIONS:
            mode = "weighted"

        return [col, direction, mode]

    def _normalized_sort_order(self) -> list[list[str]]:
        normalized: list[list[str]] = []
        for rule in self.sort_order:
            normalized_rule = self._normalize_sort_rule(rule)
            if normalized_rule is not None:
                normalized.append(normalized_rule)
        return normalized

    def _selected_sort_row(self) -> int | None:
        row = self.selected_sort_list.currentRow()
        if row < 0 or row >= len(self.sort_order):
            return None
        return row

    def _direction_label(self, direction: str) -> str:
        return SORT_DIRECTIONS.get(direction, direction)

    def _mode_label(self, mode: str) -> str:
        return SORT_MODE_LABELS.get(mode, mode)

    def _ensure_equal_mode_is_allowed(self, mode: str) -> bool:
        """Return True if a new rule can be added with the requested mode."""
        if mode != "equal":
            return True

        current = self._normalized_sort_order()
        if not current:
            QMessageBox.warning(
                self,
                "Same line needs a previous priority",
                "Same line means the column shares priority with the item above it. "
                "Add a new priority first, then add another column on the same line.",
            )
            return False

        if current[-1][2] == "strict":
            QMessageBox.warning(
                self,
                "Same line cannot follow a tie-breaker",
                "Same line belongs in the main priority blend. After a tie-breaker starts, "
                "add more tie-breakers instead.",
            )
            return False

        return True

    def _add_sort_column(self, direction: str, mode: str = "weighted") -> None:
        ...
        mode = mode if mode in SORT_MODE_DESCRIPTIONS else "weighted"

        if not self._ensure_equal_mode_is_allowed(mode):
            return

        self.sort_order = [rule for rule in self._normalized_sort_order() if rule[0] != col]
        self.sort_order.append([col, direction, mode])
        self._refresh_selected_sort_list(select_index=len(self.sort_order) - 1)
        self._clear_bid_string("Sorting priority changed. Generate the bid string again.")

    def _remove_sort_column(self) -> None:
        row = self._selected_sort_row()
        if row is None:
            return
        del self.sort_order[row]
        self._refresh_selected_sort_list(select_index=min(row, len(self.sort_order) - 1))
        self._clear_bid_string("Sorting priority changed. Generate the bid string again.")
    ...
    def _flip_selected_sort_direction(self) -> None:
        row = self._selected_sort_row()
        if row is None:
            return

        self.sort_order = self._normalized_sort_order()
        current_direction = self.sort_order[row][1]
        self.sort_order[row][1] = "asc" if current_direction == "desc" else "desc"
        self._refresh_selected_sort_list(select_index=row)
        self._clear_bid_string("Sorting direction changed. Generate the bid string again.")

    def _set_selected_sort_mode(self, mode: str) -> None:
        row = self._selected_sort_row()
        if row is None:
            return
        ...
        if mode == "equal":
            if row == 0:
                QMessageBox.warning(
                    self,
                    "Same line needs a previous priority",
                    "Same line means this row shares priority with the row above it, so it cannot be the first sorting rule.",
                )
                return
            if current[row - 1][2] == "strict":
                QMessageBox.warning(
                    self,
                    "Same line cannot follow a tie-breaker",
                    "Move this row back into the main priorities, or change it to a tie-breaker.",
                )
                return

        self.sort_order = current
        self.sort_order[row][2] = mode
        self._repair_invalid_same_line_rules()
        self._refresh_selected_sort_list(select_index=row)
        self._clear_bid_string("Sorting priority changed. Generate the bid string again.")

    def _repair_invalid_same_line_rules(self) -> None:
        """Keep the visual recipe valid after moving rows around.

        Once the tie-breaker section begins, every following rule is treated as
        a tie-breaker too. That keeps the visual recipe aligned with the backend
        meaning of strict mode.
        """
        repaired: list[list[str]] = []
        in_tie_breakers = False
        for index, rule in enumerate(self._normalized_sort_order()):
            col, direction, mode = rule

            if in_tie_breakers:
                mode = "strict"
            elif mode == "strict":
                in_tie_breakers = True
            elif mode == "equal" and (index == 0 or repaired[-1][2] == "strict"):
                mode = "weighted"

            repaired.append([col, direction, mode])
        self.sort_order = repaired
    ...
    def _build_sort_visual_text(self) -> str:
        rules = self._normalized_sort_order()
        if not rules:
            return (
                "No sorting recipe yet.\n\n"
                "Start with New priority for your most important column.\n"
                "Use Same line when two columns should share the same priority, such as Training + Blockiness.\n"
                "Use Tie-breaker for columns that should only matter after the main blend is sorted."
            )

        main_groups: list[list[str]] = []
        tie_breakers: list[str] = []
        in_tie_breakers = False

        for col, direction, mode in rules:
            label = f"{col} ({self._direction_label(direction)})"

            if mode == "strict" or in_tie_breakers:
                in_tie_breakers = True
                tie_breakers.append(label)
                continue

            if mode == "equal" and main_groups:
                main_groups[-1].append(label)
            else:
                main_groups.append([label])

        lines: list[str] = []
        if main_groups:
            lines.append("Main blended priorities:")
            for index, group in enumerate(main_groups, start=1):
                lines.append(f"{index}. " + "  +  ".join(group))
        else:
            lines.append("Main blended priorities: none")

        if tie_breakers:
            lines.append("")
            lines.append("Tie-breakers:")
            for index, label in enumerate(tie_breakers, start=1):
                lines.append(f"{index}. {label}")

        return "\n".join(lines)

    def _refresh_sort_visual_preview(self) -> None:
        if hasattr(self, "sort_visual_text"):
            self.sort_visual_text.setPlainText(self._build_sort_visual_text())

    def _refresh_selected_sort_list(self, select_index: int | None = None) -> None:
        self._repair_invalid_same_line_rules()

        self.selected_sort_list.setRowCount(0)
        priority_line = 0
        in_tie_breakers = False

        for row, (col, direction, mode) in enumerate(self.sort_order):
            self.selected_sort_list.insertRow(row)

            if mode == "strict" or in_tie_breakers:
                in_tie_breakers = True
                line_text = "Tie"
            elif mode == "equal" and priority_line > 0:
                line_text = str(priority_line)
            else:
                priority_line += 1
                line_text = str(priority_line)
            ...
            direction_item = QTableWidgetItem(self._direction_label(direction))
            mode_item = QTableWidgetItem(self._mode_label(mode))

            tooltip = SORT_MODE_DESCRIPTIONS.get(mode, "")
            for item in (line_item, col_item, direction_item, mode_item):
                item.setToolTip(tooltip)

            self.selected_sort_list.setItem(row, 0, line_item)
            self.selected_sort_list.setItem(row, 1, col_item)
            self.selected_sort_list.setItem(row, 2, direction_item)
            self.selected_sort_list.setItem(row, 3, mode_item)

        self._refresh_sort_visual_preview()

        if select_index is not None and 0 <= select_index < len(self.sort_order):
            self.selected_sort_list.selectRow(select_index)
            self.selected_sort_list.setCurrentCell(select_index, 1)

    def _use_common_sort_setup(self) -> None:
        available_by_lower = {
            self.available_columns_list.item(index).text().lower(): self.available_columns_list.item(index).text()
            for index in range(self.available_columns_list.count())
        }

        if not available_by_lower:
            QMessageBox.information(
                self,
                "Load PDFs first",
                "Load the PDFs first so the program can see which sorting columns are available.",
            )
            return

        new_sort_order: list[list[str]] = []
        missing: list[str] = []
        for column, direction, mode in COMMON_SORT_TEMPLATE:
            actual_column = available_by_lower.get(column.lower())
            if actual_column is None:
                missing.append(column)
                continue
            new_sort_order.append([actual_column, direction, mode])

        if not new_sort_order:
            QMessageBox.warning(
                self,
                "Common setup unavailable",
                "None of the common setup columns were found in this DataFrame.",
            )
            return
        ...
        if missing:
            QMessageBox.information(
                self,
                "Common setup added with missing columns",
                "The common setup was added for the columns that exist. Missing columns:\n\n" + "\n".join(missing),
            )

    def _clean_sort_order_for_columns(self, columns: list[str]) -> None:
        valid_columns = set(columns)
        old_sort_order = self._normalized_sort_order()
        self.sort_order = [rule for rule in old_sort_order if rule[0] in valid_columns]
        self._refresh_selected_sort_list()

        skipped = [rule[0] for rule in old_sort_order if rule[0] not in valid_columns]
        if skipped:
            self._write_log("Skipped saved sorting columns not found in this DataFrame: " + ", ".join(skipped))

    def _validated_sort_order_for_processing(self) -> list[tuple[str, str, str]]:
        sort_order = self._normalized_sort_order()

        for index, rule in enumerate(sort_order):
            col, direction, mode = rule

            if direction not in SORT_DIRECTIONS:
                raise ValueError(f"Invalid sorting direction for {col}: {direction}")
            if mode not in SORT_MODE_DESCRIPTIONS:
                raise ValueError(f"Invalid sorting mode for {col}: {mode}")
            if mode == "equal" and index == 0:
                raise ValueError(
                    f"{col} is the first sorting rule and cannot use Same line. "
                    "Use New priority or Tie-breaker for the first rule."
                )
            if mode == "equal" and sort_order[index - 1][2] == "strict":
                raise ValueError(
                    f"{col} is marked Same line after a tie-breaker. "
                    "Change it to New priority before the tie-breaker section, or make it a tie-breaker."
                )

        return [(col, direction, mode) for col, direction, mode in sort_order]
    ...
    def _validate_sorting_settings_values(
        ...
        # default_mode is kept only for backward compatibility with older
        # sort_dataframe_by_conditions versions. The new GUI stores the actual
        # hybrid mode directly on each selected sorting row.
        default_mode = (default_mode or "weighted").strip().lower() or "weighted"
        ...
        if weighting_style == "equal":
            # Older configs could save global equal weighting. In the hybrid
            # system, equal is a per-row mode, so use soft as the safest global curve.
            weighting_style = "soft"

        if weighting_style not in WEIGHTING_STYLE_DESCRIPTIONS:
            raise ValueError("Weighted style must be soft or hard.")
    ...
    def _apply_sorting_settings_to_ui(self, sorting_settings: dict[str, Any]) -> None:
        self.default_mode = str(sorting_settings.get("default_mode", "weighted"))
        self.weighting_style = str(sorting_settings.get("weighting_style", DEFAULT_SORTING_SETTINGS["weighting_style"]))
        if self.weighting_style not in WEIGHTING_STYLE_DESCRIPTIONS:
            self.weighting_style = DEFAULT_SORTING_SETTINGS["weighting_style"]
        self.soft_max_weight = float(sorting_settings.get("soft_max_weight", DEFAULT_SORTING_SETTINGS["soft_max_weight"]))
        self.soft_min_weight = float(sorting_settings.get("soft_min_weight", DEFAULT_SORTING_SETTINGS["soft_min_weight"]))
        self.keep_score_columns = bool(sorting_settings.get("keep_score_columns", DEFAULT_SORTING_SETTINGS["keep_score_columns"]))
    ...
    def _save_sorting_settings(self, sorting_settings: dict[str, Any], *, show_message: bool = True) -> None:
        ...
        self._write_log(
            ...
            f"weighted style {sorting_settings['weighting_style']}, "
            f"soft weights {sorting_settings['soft_min_weight']}–{sorting_settings['soft_max_weight']}, "
            f"keep score columns {sorting_settings['keep_score_columns']}."
        )
    ...
    def _open_sorting_settings_dialog(self) -> None:
        ...
        intro = QLabel(
            "Hybrid mode is now selected directly in the Sorting table. "
            "These settings only control the auto-weight curve and export helper columns."
        )
        ...
        soft_max_entry.setToolTip("Used by the soft weighted style. This is the weight given to the first weighted item. Default: 3.0.")
        soft_min_entry = QLineEdit(str(current["soft_min_weight"]))
        soft_min_entry.setToolTip("Used by the soft weighted style. This is the weight given to the last weighted item. Default: 1.0.")
        ...
        keep_score_columns_check.setToolTip("When enabled, extra score/helper columns created by sorting remain in the exported Excel file.")
        ...
        add_row(1, "Weighted style:", weighting_style_combo, weighting_tip)
        add_row(2, "Soft max weight:", soft_max_entry, soft_max_entry.toolTip())
        add_row(3, "Soft min weight:", soft_min_entry, soft_min_entry.toolTip())
        layout.addWidget(keep_score_columns_check, 4, 0, 1, 2)
    ...
    def _collect_inputs(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sort_order": self._validated_sort_order_for_processing(),
    ...
    def _build_dataframe(self, inputs: dict[str, Any], *, apply_sort: bool) -> tuple[pd.DataFrame, list[dict[str, str]] | None]:
        ...
        if apply_sort and inputs["sort_order"]:
            ...
            sort_order_preview = ", ".join(
                f"{col} {direction} {mode}"
                for col, direction, mode in inputs["sort_order"]
            )
            self._log(
                "Sorting DataFrame with hybrid sort order "
                f"({sort_order_preview}); weighted style {sorting_settings['weighting_style']}..."
            )

            sort_function = getattr(pf, "sort_dataframe_by_conditions", sort_dataframe_by_conditions)
            sort_kwargs: dict[str, Any] = {}

            try:
                signature = inspect.signature(sort_function)
            ...
            optional_kwargs = {
                "default_mode": sorting_settings.get("default_mode", "weighted"),
            ...
            for key, value in optional_kwargs.items():
                if accepts_kwargs or key in parameters:
                    sort_kwargs[key] = value

            df = sort_function(df, inputs["sort_order"], **sort_kwargs)
