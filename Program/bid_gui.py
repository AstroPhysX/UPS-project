...
This version adds:
    ...
    - Windows app/window icon support using app_icon.ico
    - TRIPS PDF extraction progress bar with percentage
    - Saved number-of-lines-to-bid field and copyable bid string output
    - Open Visualizer button that opens BidSpreadsheetViewer from excel_killer.py
    - Export-complete dialog with buttons to open the Excel file or its folder
"""
...
try:
    ...
    EXCEL_KILLER_IMPORT_ERROR = None
...
def set_windows_app_id(app_id: str = "UPS.BidAnalyzer.App") -> None:
    """Help Windows use the app icon in the taskbar instead of Tk's default icon."""
...
def apply_window_icon(window: tk.Tk) -> None:
    """Apply app_icon.ico to the Tk window if the icon file is available."""
...
def validate_positive_int(value: str, field_name: str) -> int:
    """Validate a positive whole-number entry from the GUI."""
...
class BidGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.trip_progress_var = tk.DoubleVar(value=0.0)
        self.trip_progress = ttk.Progressbar(
            file_frame,
            mode="determinate",
            maximum=100,
            variable=self.trip_progress_var,
        )
        self.trip_progress.grid(
            row=3,
            column=0,
            columnspan=4,
            sticky="ew",
        ...
        self.trip_progress_text_var = tk.StringVar(value="Trips extraction progress: not started.")
        ttk.Label(file_frame, textvariable=self.trip_progress_text_var).grid(
            row=4,
            column=0,
            columnspan=4,
            sticky="w",
        ...
        self.training_start_var.trace_add("write", lambda *_: self._mark_bid_string_stale("Training dates changed. Generate the bid string again after sorting."))
        ...
        self.training_end_var.trace_add("write", lambda *_: self._mark_bid_string_stale("Training dates changed. Generate the bid string again after sorting."))
        ...
        bid_frame = ttk.LabelFrame(container, text="Copy Line Numbers", padding=10)
        bid_frame.grid(row=5, column=0, sticky="ew", pady=(0, 10))
        bid_frame.columnconfigure(0, weight=1)

        # Keep the number entry in its own row so the long label does not force
        # the Bid string box to start far away from the short "Bid string:" label.
        number_row = ttk.Frame(bid_frame)
        number_row.grid(row=0, column=0, sticky="ew", pady=4)
        ttk.Label(number_row, text="Number of lines you would like to bid:").pack(side="left", padx=(0, 8))
        self.number_of_lines_var = tk.StringVar(value=str(DEFAULT_NUMBER_OF_LINES_TO_BID))
        self.number_of_lines_var.trace_add("write", lambda *_: self._mark_bid_string_stale())
        ttk.Entry(number_row, textvariable=self.number_of_lines_var, width=10).pack(side="left")

        bid_string_row = ttk.Frame(bid_frame)
        bid_string_row.grid(row=1, column=0, sticky="ew", pady=4)
        bid_string_row.columnconfigure(1, weight=1)

        ttk.Label(bid_string_row, text="Bid string:").grid(row=0, column=0, sticky="nw", padx=(0, 8), pady=(3, 0))

        # A Text widget is better than a readonly Entry here because the bid
        # string can become very long when bidding 100+ lines. It wraps, grows
        # horizontally with the window, and has a scrollbar if needed.
        self.bid_string_text = tk.Text(
            bid_string_row,
            height=5,
            wrap="word",
            state="disabled",
        ...
        self.bid_string_text.grid(row=0, column=1, sticky="ew")

        bid_string_scrollbar = ttk.Scrollbar(
            bid_string_row,
            orient="vertical",
            command=self.bid_string_text.yview,
        )
        bid_string_scrollbar.grid(row=0, column=2, sticky="ns")
        self.bid_string_text.configure(yscrollcommand=bid_string_scrollbar.set)

        bid_buttons = ttk.Frame(bid_frame)
        bid_buttons.grid(row=2, column=0, sticky="e", pady=(6, 0))
        self.generate_bid_string_button = ttk.Button(
            bid_buttons,
            text="Generate bid string",
            command=self.generate_bid_string,
        )
        self.generate_bid_string_button.pack(side="left", padx=(0, 6))
        self.copy_bid_string_button = ttk.Button(
            bid_buttons,
            text="Copy bid",
            command=self.copy_bid_string,
        )
        self.copy_bid_string_button.pack(side="left")

        self.bid_string_status_var = tk.StringVar(value="Bid string not generated yet.")
        ttk.Label(bid_frame, textvariable=self.bid_string_status_var).grid(
            row=3,
            column=0,
            sticky="w",
            pady=(4, 0),
        ...
        self.visualizer_button = ttk.Button(
            action_frame,
            text="Open Visualizer",
            command=self.open_visualizer,
        )
        self.visualizer_button.grid(row=0, column=1, padx=(0, 8))
    ...
    def _load_saved_values(self) -> None:
        ...
        try:
            ...
            saved_number_of_lines = DEFAULT_NUMBER_OF_LINES_TO_BID
        self.number_of_lines_var.set(str(saved_number_of_lines))
    ...
    def _reset_trip_progress(self, message: str = "Trips extraction progress: starting...") -> None:
        """Reset the TRIPS extraction progress bar from the main Tk thread."""
        self.trip_progress_var.set(0.0)
        self.trip_progress_text_var.set(message)

    def _queue_trip_progress(self, progress_data: dict[str, Any]) -> None:
        """Receive progress updates from extract_trips_from_pdf safely.

        The extractor runs in a background thread, so it must not touch Tkinter
        widgets directly. It sends progress dictionaries here, then the main
        GUI thread applies them inside _poll_queue().
        """
        self.message_queue.put(("trip_progress", progress_data))

    def _handle_trip_progress(self, progress_data: dict[str, Any]) -> None:
        """Update the TRIPS progress bar and percentage label."""
        try:
        ...
        self.trip_progress_var.set(percent)
        ...
        self.trip_progress_text_var.set(label)

    def _set_busy(self, busy: bool) -> None:
        state = "disabled" if busy else "normal"
        self.load_button.configure(state=state)
        self.export_button.configure(state=state)
        if hasattr(self, "visualizer_button"):
            self.visualizer_button.configure(state=state)

        if hasattr(self, "generate_bid_string_button"):
            self.generate_bid_string_button.configure(state=state)
        if hasattr(self, "copy_bid_string_button"):
            self.copy_bid_string_button.configure(state=state)
            ...
            while True:
                ...
                if event == "log":
                    ...
                    self._set_bid_string(
                        bid_string,
                        f"Bid string ready for {number_of_lines} lines."
                        if number_of_lines
                        else "Bid string ready.",
                    )
                    self.pdf_status_var.set("Bid string ready.")
                    ...
                    try:
                        ...
                        self.pdf_status_var.set("Visualizer error. See status box below.")
                        self._write_log(f"ERROR opening visualizer: {exc}")
                        messagebox.showerror("Visualizer error", str(exc))
    ...
    def _open_containing_folder(self, file_path: str | Path) -> None:
        """Open the folder containing a file. On Windows, select the file."""
        path = Path(file_path)
        folder = path.parent
    ...
    def _show_export_complete_dialog(self, output_path: str | Path | None) -> None:
        """Show export-complete dialog with file/folder action buttons."""
        if output_path is None:
            messagebox.showinfo("Finished", "Excel file created.")
            return
        ...
        path_entry_var = tk.StringVar(value=str(path))
        path_entry = ttk.Entry(main, textvariable=path_entry_var, width=90, state="readonly")
        path_entry.grid(row=1, column=0, sticky="ew", pady=(0, 12))
        ...
        ttk.Button(
            ...
            command=lambda: self._open_file_with_default_app(path),
            style="Green.TButton",
        ...
        ttk.Button(
            buttons,
            text="Open folder",
            command=lambda: self._open_containing_folder(path),
        ).pack(side="left", padx=(0, 8))

        ttk.Button(buttons, text="Close", command=dialog.destroy).pack(side="left")

        dialog.bind("<Return>", lambda _event: self._open_file_with_default_app(path))
        dialog.bind("<Escape>", lambda _event: dialog.destroy())
    ...
    def _mark_bid_string_stale(self, status_message: str | None = None) -> None:
        """Mark the current bid string as outdated after a user-editable value changes."""
        self._clear_bid_string(
            status_message or "Inputs changed. Generate the bid string again after sorting."
        )

    def _display_bid_string(self, bid_string: str) -> None:
        """Display the bid string in the multi-line Text box."""
        if hasattr(self, "bid_string_text"):
            self.bid_string_text.configure(state="normal")
            self.bid_string_text.delete("1.0", tk.END)
            if bid_string:
                self.bid_string_text.insert("1.0", bid_string)
            self.bid_string_text.configure(state="disabled")
        elif hasattr(self, "bid_string_var"):
            # Compatibility fallback if an older one-line Entry is restored.
            self.bid_string_var.set(bid_string)

    def _get_displayed_bid_string(self) -> str:
        """Return the bid string currently visible in the GUI."""
        if hasattr(self, "bid_string_text"):
            return self.bid_string_text.get("1.0", tk.END).strip()
        if hasattr(self, "bid_string_var"):
            return self.bid_string_var.get().strip()
        return ""

    def _clear_bid_string(self, status_message: str | None = None) -> None:
        """Clear the displayed bid string when inputs that affect ranking change."""
        self.latest_bid_string = ""
        self._display_bid_string("")

        if status_message and hasattr(self, "bid_string_status_var"):
            self.bid_string_status_var.set(status_message)

    def _set_bid_string(self, bid_string: str, status_message: str = "Bid string ready.") -> None:
        """Store and display the latest generated bid string."""
        self.latest_bid_string = bid_string
        self._display_bid_string(bid_string)
        self.bid_string_status_var.set(status_message)

    def _copy_bid_string_to_clipboard(self) -> None:
        """Copy the already-generated bid string to the OS clipboard."""
        bid_string = self.latest_bid_string.strip() or self._get_displayed_bid_string()

        if not bid_string:
            messagebox.showinfo("Bid string", "Generate the bid string first.")
            return

        self.clipboard_clear()
        self.clipboard_append(bid_string)
        self.update_idletasks()
        self.bid_string_status_var.set("Bid string copied to clipboard.")

    def generate_bid_string(self, *, copy_after: bool = False) -> None:
        """Build the sorted DataFrame and generate the line-number bid string."""
        try:
        ...
        if self.cached_pdf_key != current_key:
            self.pdf_status_var.set("PDFs not loaded for these paths. Loading first, then generating bid string...")
            self._reset_trip_progress("Trips extraction progress: waiting to start...")

        self.bid_string_status_var.set("Generating bid string...")
        self._start_worker(self._bid_string_worker, inputs, copy_after)

    def copy_bid_string(self) -> None:
        """Copy the bid string, generating it first if needed."""
    ...
    def open_visualizer(self) -> None:
        """Build the sorted DataFrame and open the simple visualizer."""
        try:
            inputs = self._collect_inputs()
            self._save_inputs_to_config(inputs)
        except Exception as exc:
        ...
        current_key = (inputs["trips_pdf_path"], inputs["lines_pdf_path"])
        if self.cached_pdf_key != current_key:
            self.pdf_status_var.set("PDFs not loaded for these paths. Loading first, then opening visualizer...")
            self._reset_trip_progress("Trips extraction progress: waiting to start...")
        else:
            self.pdf_status_var.set("Opening visualizer...")

        self._start_worker(self._visualizer_worker, inputs)

    def _extract_trips_with_progress(self, trips_pdf_path: str) -> dict[str, Any]:
        """Extract trips and forward page-progress updates to the GUI.

        The fallback keeps the GUI compatible with older versions of
        extract_trips_from_pdf that do not yet accept progress_callback.
        """
        try:
    ...
    def _get_bid_spreadsheet_viewer_class(self) -> Any:
        """Return BidSpreadsheetViewer, or raise a clear import error."""
        global BidSpreadsheetViewer, EXCEL_KILLER_IMPORT_ERROR

        if BidSpreadsheetViewer is not None:
            return BidSpreadsheetViewer

        try:
            from excel_killer import BidSpreadsheetViewer as ImportedBidSpreadsheetViewer
        except Exception as exc:
            original_error = EXCEL_KILLER_IMPORT_ERROR or exc
            raise RuntimeError(
                "Could not import BidSpreadsheetViewer from excel_killer.py. "
                "Make sure excel_killer.py is in the same folder as this GUI file. "
                f"Original error: {original_error}"
            ) from exc

        BidSpreadsheetViewer = ImportedBidSpreadsheetViewer
        EXCEL_KILLER_IMPORT_ERROR = None
        return BidSpreadsheetViewer

    def _open_visualizer_from_payload(self, payload: Any) -> None:
        """Open BidSpreadsheetViewer in a separate Toplevel window."""
        ...
        viewer_window.title("Bid Table Viewer")
        viewer_window.geometry("1400x800")
        viewer_window.minsize(900, 500)

        try:
            apply_window_icon(viewer_window)  # type: ignore[arg-type]
        except Exception:
            pass

        viewer = viewer_class(viewer_window)
        viewer.pack(fill="both", expand=True)

        # Keep an explicit reference on the Toplevel so Tkinter/Python do not
        # accidentally garbage-collect the viewer object.
        viewer_window.viewer = viewer  # type: ignore[attr-defined]
        ...
        self.pdf_status_var.set("Visualizer opened.")
