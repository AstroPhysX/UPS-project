...
class BidGUI(QMainWindow):
    ...
    def _mark_pdf_paths_changed(self) -> None:
        current_key = (
            self.trips_path_edit.text().strip(),
            self.lines_path_edit.text().strip(),
        )

        if self.analysis_pipeline.invalidate_if_paths_changed(*current_key):
            ...
            self._clear_bid_string(
                "PDF paths changed. Generate the bid string again "
                "after loading/sorting."
            )
    ...
    def _get_bid_spreadsheet_viewer_class(self) -> Any:
        ...
        try:
            from bid_analyzer.gui.GUI_spreadsheet_pyside6 import BidSpreadsheetWindow
            return BidSpreadsheetWindow
        except Exception as exc:
            errors.append(f"GUI_spreadsheet_pyside6.BidSpreadsheetWindow: {exc}")

        try:
            from bid_analyzer.gui.GUI_spreadsheet_pyside6 import BidSpreadsheetViewer
    ...
    def _open_visualizer_from_payload(self, payload: Any) -> None:
        ...
        try:
            # Works with BidSpreadsheetWindow from bid_analyzer.gui.GUI_spreadsheet_pyside6.py.
